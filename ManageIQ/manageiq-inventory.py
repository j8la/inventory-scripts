...
def main():
    ...
    if error_count == 0:
        ...
        hList = req(server, \
                    '/api/hosts?expand=resources&attributes=' \
                    'id,' \
                    'hostname,' \
                    'ipaddress,' \
                    'vmm_vendor,' \
    ...
    if error_count == 0:
        ...
        hList = req(server, \
                '/api/vms?expand=resources&attributes=' \
                'name,' \
                'id,' \
                'ipaddresses,' \
                'vendor,' \
        ...
        if hList != -1:
            for h in hList["resources"]:
                ...
                if h["name"] not in vm_filter:

                    hosts['VirtualMachines']['hosts'].append(h["name"])

                    try:
                        ip = h["ipaddresses"][h["ipaddresses"].__len__()-1]
                    except IndexError:
                        ip = 'null'
